...
Note the last step, i.e., `ninja` (`all` target), also generates a package for correctness testing, which is performed by [Joshua testing framework](https://github.com/FoundationDB/fdb-joshua/). The package is a file at `<PATH_TO_BUILD_DIRECTORY>/packages/` directory, named as `correctness-<VERSION>.tar.gz`.
Specifically, the correctness package is generated by `package_tests` target for `ninja`.
