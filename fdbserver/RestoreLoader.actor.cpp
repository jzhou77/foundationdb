...
ACTOR Future<Void> _processLoadingParam(LoadingParam param, Reference<RestoreLoaderData> self) {
	...
	bool inserted;
	std::tie(kvOpsPerLPIter, inserted) = self->kvOpsPerLP.emplace(param, VersionedMutationsMap());
	ASSERT(inserted);
	std::tie(samplesIter, inserted) = self->sampleMutations.emplace(param, MutationsVec());
	ASSERT(inserted);
