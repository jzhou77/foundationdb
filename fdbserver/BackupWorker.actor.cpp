...
struct BackupData {
	...
	AsyncVar<std::vector<std::pair<UID, Version>>> backupIDs; // active backup (UID, StartVersion) pairs
		...
		loop {
			try {
				...
				if (value.present() && started) {
					self->backupIDs.set(decodeBackupStartedValue(value.get()));
					TraceEvent e("BackupWorkerGotStartKey", self->myId);
					int i = 1;
					for (auto uidVersion : self->backupIDs.get()) {
...
ACTOR Future<Void> monitorAllWorkerStarted(BackupData* self) {
	loop {
		while (self->backupIDs.get().empty()) {
			wait(self->backupIDs.onChange());
		}
		...
		if (tagVersions.size() == self->logSystem.get()->getLogRouterTags()) {
			...
			for (const auto uidVersion : self->backupIDs.get()) {
				bool backed = true;
		...
		wait(delay(SERVER_KNOBS->WORKER_LOGGING_INTERVAL/2.0) || self->backupIDs.onChange());
	}
}
